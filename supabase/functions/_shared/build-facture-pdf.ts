...
export async function buildInvoicePdf(facture: any, tenant: any, settings: Record<string, string>) {
  ...
  async function embedLogo(url: string) {
    try {
      ...
      // garde-fou : un logo trop lourd (upload ancien, avant le redimensionnement automatique
      // côté client) peut faire gonfler chaque PDF au point de faire échouer un export groupé
      // (ex: ZIP de toutes les factures d'un mois) — on l'omet plutôt que de risquer un crash.
      if (bytes.length > 800_000) return null;
